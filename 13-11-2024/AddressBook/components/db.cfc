		...
		<cfif StructKeyExists(arguments, "contactId") AND arguments.contactId NEQ "">
			...
			<cfquery>
        			DELETE FROM user_hobbies
       				WHERE contact_id = <cfqueryparam value="#local.decryptedId#" cfsqltype="cf_sql_integer">
    			</cfquery>
	...
	<cffunction name="getPages" access="public" returntype="query">
		...
		<cfset local.decryptedId = decryptId(arguments.contactId)>
			...
			<cfquery name="deleteContactData">
            			DELETE FROM contact 
            			WHERE idcontact = <cfqueryparam value="#local.decryptedId#" cfsqltype="cf_sql_integer">
			</cfquery>
