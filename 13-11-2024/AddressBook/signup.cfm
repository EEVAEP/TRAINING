<cfif structKeyExists(form, 'submit')>
	...
	<cftry>
	<cfset variables.validationErrors = application.userService.validateRegisterInput(form.fullname, form.email, form.username, form.password, form.confirmPassword)>

	<cfif structKeyExists(variables, "validationErrors") AND arrayLen(validationErrors) EQ 0>
		<cfset local.result = application.userService.registerUser(form.fullname, form.email, form.username, form.password)>
