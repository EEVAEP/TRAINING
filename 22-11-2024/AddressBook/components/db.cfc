<cfcomponent>
	...
	<cffunction name="validateAddEditContactDetails" access="remote" returntype="any" returnformat="JSON">
		...
		<cfargument name="is_public" type="string" required="false">
		...
		<cfset local.validTitles = []>
    		...
    		<cfset local.allowedStaticTitles = ["Mr", "Mrs", "Ms", "Dr", "Miss"]>

   
    		<cfif NOT arrayContains(local.validTitles, arguments.title) AND NOT arrayContains(local.allowedStaticTitles, arguments.title)>
        		<cfset arrayAppend(local.errors, "*The title must be one of the following: " & arrayToList(local.validTitles, ", ") & " or " & arrayToList(local.allowedStaticTitles, ", "))>
    		</cfif>
		...
		<cfset local.allowedStaticGender = ["Male", "Female", "Agender", "Transgender", "Bisexual"]>
		<cfif NOT arrayContains(local.validGender, arguments.gender) AND NOT arrayContains(local.allowedStaticGender, arguments.gender)>
			<cfset arrayAppend(local.errors, "*Enter a valid gender")>
		</cfif>
	...
	<!---<cffunction name="excelUploadedFile" access="remote" returnformat="JSON">
		...
		<cftry>
			...
			<cfset local.uploadedFilePath = local.uploadPath & local.fileUploadResult.serverFile>

			<cfset local.spreadsheetObj = SpreadsheetRead(local.uploadedFilePath)>
        		<!---<cfset local.totalRows = SpreadsheetGetRowCount(local.spreadsheetObj)>--->
			
			


	
			<cfset local.response = {status="success", message="File uploaded successfully", debugInfo = {
            														uploadPath = local.uploadPath,
            														uploadedFilePath = local.uploadedFilePath,
															spreadsheet = local.spreadsheetObj
