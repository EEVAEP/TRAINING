...
$(document).ready(function() {
	...
	$(document).on('click', '.edit', function() {
		...
		let dob = new Date(contact.dob);
			...
			const selectedHobbies = contact.hobbies.map(hobby => hobby.hobbyId);

			for (let i = 0; i < hobbiesSelect.options.length; i++) {
        			const option = hobbiesSelect.options[i];
        			if (selectedHobbies.includes(parseInt(option.value))) {
